...
contract FarmCore is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuard{
    ...
    function sendToken(
        ...
        //用于添加流动性
        toAddLiquidity = amountUsdt * 60 / 100;
        //层级奖励
        uint256 toHierarchy = amountUsdt * 15 / 100;
        //两个钱包的分发
        uint256 toCommunity = amountUsdt * 2 / 100;
        uint256 toBuyback   = amountUsdt * 3 / 100;
        uint256 totalUsed = toAddLiquidity + toHierarchy + toCommunity + toBuyback;
        totalCumulateAward += (amountUsdt - totalUsed);
    ...
    function calcIssueAmount() external onlyAdmin(){
        //总奖励的20%
        uint256 percent20 = totalCumulateAward * 20 / 100;
        //3万usdt奖励占其中的百分比 = 15 / 25 * 100 
        uint256 toThirtyThousand = percent20 * 60 / 100;
        uint256 toOther = percent20 - toThirtyThousand;
        cumulateAwardForOther[rounds][Process.Cumulate.THIRTY_THOUSAND_RANK] = toThirtyThousand;
        
        //1万usdt奖励占比toOther = 20 / 50 * 100、
        uint256 toTenThousand = toOther * 40 / 100;
        cumulateAwardForOther[rounds][Process.Cumulate.TEN_THOUSAND_RANK] = toTenThousand;
        uint256 toTodayTop = toOther * 20 / 100;
        cumulateAwardForOther[rounds][Process.Cumulate.TODAY_TOP] = toTodayTop;
        uint256 toNode = toOther - toTenThousand - toTodayTop;
        uint256 toSamll = toNode * 40 / 100;
        uint256 toLarge = toNode - (toNode * 80 / 100); 
        cumulateAwardForNode[rounds][Process.NodeType.SMALL] = toSamll;
        cumulateAwardForNode[rounds][Process.NodeType.MEDIUM] = toSamll;
        cumulateAwardForNode[rounds][Process.NodeType.LARGE] = toLarge;
        rounds++;
    }

    function issueTenThousandRankAward() public {
        uint256 amount = cumulateAwardForOther[rounds - 1][Process.Cumulate.TEN_THOUSAND_RANK];
        farmReferral.issueTenThousandUsdtRankAward(amount);
        cumulateAwardForOther[rounds - 1][Process.Cumulate.TEN_THOUSAND_RANK] = 0;
    }
    
    function issueThirtyThousandRankAwardForAnc() public {
        ...
        farmReferral.issueThirtyThousandUsdtRankAward(Process.Token.ANC_TOKEN, amountAnc - toStaking);
    }

    function issueThirtyThousandRankAwardForUsdt() public {
        uint256 amount = cumulateAwardForOther[rounds - 1][Process.Cumulate.THIRTY_THOUSAND_RANK];
        farmReferral.issueThirtyThousandUsdtRankAward(Process.Token.USDT_TOKEN, amount);
        cumulateAwardForOther[rounds - 1][Process.Cumulate.THIRTY_THOUSAND_RANK] = 0;
    }

    function issueTodayTopUsdtAward() public {
        farmToday.issueTodayTopAward(cumulateAwardForOther[rounds - 1][Process.Cumulate.TODAY_TOP]);
        cumulateAwardForOther[rounds - 1][Process.Cumulate.TODAY_TOP] = 0;
    }
    ...
    function issueSmallNodeAward() public {
        ...
        amounts[0] = cumulateAwardForNode[rounds - 1][Process.NodeType.SMALL];
    ...
    function issueOtherNodeAward() public {
        ...
        amounts[0] = cumulateAwardForNode[rounds - 1][Process.NodeType.MEDIUM];
        amounts[1] = cumulateAwardForNode[rounds - 1][Process.NodeType.LARGE];
