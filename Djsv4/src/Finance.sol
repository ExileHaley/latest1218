...
contract Finance is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuard, IFinance{
    ...
    function setRouterAddr(address _router) external onlyOwner{
        router = _router;
    }
    ...
    function referral(address recommender, address user) external nonReentrant onlyRouter{
        //需要映射
        // if(whetherNeedMigrate(msg.sender)) revert Errors.NeedMigrate();
        require(initialCode != user);
        if(recommender == address(0)) revert Errors.ZeroAddress();
        if(recommender == user) revert Errors.InvalidRecommender();
        ...
        if(referralInfo[user].recommender != address(0)) revert Errors.InviterExists();
        referralInfo[user].recommender = recommender;
        emit Referrals(recommender, user);
    }

    function swapSubToken(address user, uint256 amountUSDT) external onlyRouter{
        uint256 fee = amountUSDT * 3 / 100;
        TransferHelper.safeTransferFrom(USDT, user, recipientForBurn, fee);

        if(amountUSDT > referralInfo[user].subCoinQuota) revert Errors.InsufficientQuota();
        TransferHelper.safeTransferFrom(USDT, user, liquidityManager, amountUSDT - fee);

        ILiquidityManager(liquidityManager).swapForSubTokenToUser(user, amountUSDT -fee);
        referralInfo[user].subCoinQuota -= amountUSDT;
    }

    function stake(address user, uint256 amountUSDT) external nonReentrant onlyRouter{
        ...
        TransferHelper.safeTransferFrom(USDT, user, liquidityManager, amountToAddLiquidity);
        TransferHelper.safeTransferFrom(USDT, user, recipientForBurn, amountToBurnSubToken);
        TransferHelper.safeTransferFrom(USDT, user, nodeDividends, amountToNode);
        ...
        //更新用户质押收益、share等级收益
        _settleStakingReward(user);

        //质押数量更新
        // u.stakingUsdt += amountUSDT;
        if(_isOut(user)) {
            _handleOutUserReward(user);
        ...
        uint256 num = 0;
        if(!isAddDirectReferrals[user]){
            num = 1;
            directReferrals[r.recommender].push(user);
            isAddDirectReferrals[user] = true;
        }

        processUpgrade(user, amountUSDT, num);
        emit Staked(user, amountUSDT);
    }

    function claim(address user) external nonReentrant Pause onlyRouter{
        Process.User storage u = userInfo[user];
        if (u.stakingUsdt == 0 && user != initialCode) revert Errors.NoStake();
        ...
        uint256 amount = getUserAward(user);
        ...
        // 6️⃣ 发放 USDT
        ILiquidityManager(liquidityManager).acquireSpecifiedUsdt(user, amount);
